...
function main(file)
    ...
    for line in eachline(f)
        opp, res = split(line, " ")
        opp = Int(only(opp)) - Int('A') # get opponent play (0=rock, 1=paper, 2=scissors)
        res = Int(only(res)) - Int('Y') # get intended end  (-1=loss, 0=draw, 1=win)

        def = mod(opp + res, 3) + 1     # find our play (0,1,2) then add 1 to get score for it
