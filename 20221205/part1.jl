...
crates get crushed or fall over, the crane operator will rearrange them in a series of carefully-
planned steps. After the crates are rearranged, the desired crates will be at the top of each stack.
    ...
    for line in eachline(f)
        if !move    # start of stack commands haven't been reached
            ...
            if line != ""   # if line not blank
                ...
                # pop items from the sending stack and push them to the receiving stack
                for i in range(1, cmd[1])
                    push!(stacks[cmd[3]], pop!(stacks[cmd[2]]))
                end
            end
        end
    end
