...
function main(file)
    ...
    for (i, line) in enumerate(eachline(f)) # go through each line
        if (i-1) % 3 == 0   # demonstrates two different ways to iterate over characters in string
            chars = map(c -> Int(c), collect(line))
        else
            chars = intersect(chars, [Int(c) for c in line])
        end

        if (i-1) % 3 == 2   # add to score if we're at the last of the triplet
            pri = chars[1]
            sum += islowercase(Char(chars[1])) ? pri - Int('a') + 1 : pri - Int('A') + 27
        end
    end
