...
function traverse!(tot::Vector{Int128}, dir::Dir)::Int128
    #= computes size of directory recursively, adds size to tot[1] if <=100,000 =#
    s = sum(dir.files)
    for subdir in dir.subdirs
        s += traverse!(tot, subdir)
    end

    s <= 100000 && (tot[1] += s)    # add to total tracker if less than 100,000
    return s
end
...
function main(file)
    ...
    total = Vector{Int128}([0])
    s = traverse!(total, dirs["/"])
    

    println("Total size of root: $s")   # ($stotal)")
    println("Sum of dirs < 100k: $(total[1])")
end
