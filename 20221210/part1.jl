...
The interesting signal strengths can be determined as follows:
  ...
  - During the 20th cycle, register X has the value 21, so the signal strength is 20 * 21 = 420. 
    ...
    starting value, 1, plus all of the other addx values up to that point: 1 + 15 - 11 + 6 - 3 + 5 
    - 1 - 8 + 13 + 4 = 21.)
...
function main(file)
    f = open(file)
    cycle = 0   # starting cycle
    x = 1       # starting value of register X
    sum = 0     # sum of signal strengths

    for line in eachline(f)
        if startswith(line, "noop")
            cycle += 1  # increment cycles (noop takes 1)
            (cycle - 20) % 40 == 0 && (sum += cycle * x)

        elseif startswith(line, "addx")
            _, num = split(strip(line), " ")
            num = parse(Int, num)   # get number to add

            cycle += 2  # increment cycles (addx takes 2)
            # interesting cycle occurred; may have passed it so subtract one if we did
